  ...
  return (
    <div className="grid grid-cols-1 tablet:grid-cols-2 pt-[80px] desktop:pt-[96px] pl-5 pr-5">
      ...
      <div className="whitespace-pre-wrap text-[16px]/[18px] pt-3 tablet:pt-0 pl-[6px] laptop:pl-6 desktop:pl-[65px] laptop:pr-[51px] desktop:pr-[133px] overflow-y-auto scrollbar-hide tablet:max-h-[calc(100vh-80px)] desktop:max-h-[calc(100vh-96px)] pb-5">
        <div className="font-bagossTrial text-[20px]/[22px] desktop:text-[25px]/[26px] items-center pb-4 hidden tablet:flex ">
          {selectedNewsData.title}
          <span className="text-[9px] tablet:text-[12px] pl-2">{`(${selectedNewsData.gallery.length <= 9 ? `0${selectedNewsData?.gallery?.length}` : `${selectedNewsData?.gallery?.length}`})`}</span>
        </div>
